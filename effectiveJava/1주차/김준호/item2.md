# 아이템 2: 생성자 매개변수가 많은 경우에 빌더 를 고려하라
...
자바는 freeze 메서드를 제공하지 않는다. 직접 구현해야한다.
...
빌더의 생성자나 메소드에서 유효성 확인을 할 수도 있고 여러 매개변수를 혼합해서 확인해야 하는 경우에는 `build` 메소드에서 호출하는 생성자에서 할 수 있다. 빌더에서 매개변수를 객체로 복사해온 다음에 확인하고, 검증에 실패하면 `IllegalArgumentException`을 던지고 에러 메시지로 어떤 매개변수가 잘못됐는지 알려줄 수 있다.
